...
- **KRISS 기준 시각 표시** — KRISS 보정 신뢰 시각을 대형 시계로 표시하고, **전체화면 모드**(벽시계 용도·화면 절전 억제)를 제공. 미검증(동기 실패) 상태는 명확히 구분 표시
...
- **동기화 설정 + 표준 변경 이력** — 폴링 주기·허용 한계를 화면에서 조정하고, 변경 시 **사유와 함께 버전별 이력**을 기록
- **허용 한계 초과 로그** — 한계 초과 이벤트를 장비·측정 오프셋·당시 한계·시각과 함께 기록(진행 중 ↔ 해제됨). **최근 1주일 / 지난 이력(기간 선택 조회)** 탭 제공
...
- **저장소**: SQLite (SQLAlchemy, 구현 완료 — 재시작 후 데이터 유지) · 운영 시 PostgreSQL 전환
...
- 전체 산출물을 한 파일로 보려면 [`docs/validation-package.html`](docs/validation-package.html) — 사이드바 목차·인쇄(PDF) 가능, `python docs/build_html.py` 로 재생성.
- 운영자용 [장비 NTP 설정 확인·등록 매뉴얼](docs/manual-device-ntp-setup.md) (MAN-001).
