...
| BP-4 드리프트·폴링 | URS-020, URS-022 | FS-020,022,024 / DS-001,030 | RISK-001,003,004 | OQ-020,022,025 | ✅ 측정·경고·STALE·주기 스케줄러(FS-024) 구현 / 자동 *보정* 로깅은 비해당(읽기 전용 감시) |
| BP-5 변조방지 | URS-003, URS-040 | FS-002,040 / DS-010 | RISK-002,006 | OQ-002,040 | ✅ 표준 버전관리·변경 이력(값 스냅샷·사유) / ⚠️ 전 엔터티 감사추적·전자서명은 범위 제외(한계 초과 로그로 대체, who=system) |
...
- 초기 분석의 후속 우선순위 중 **영속 저장(SQLite)·주기 폴링 스케줄러(FS-024)는 구현 완료**, 전체 감사추적은 **범위 제외**(한계 초과 로그로 대체) 결정으로 정리되었다. 남은 규제적 후속 과제는 **① KRISS 소스 주소 검토(BP-1, `time.kriss.re.kr` 응답 확인됨 — 공식 권고 주소 정합 여부만 점검)** 및 운영 전환 시 PostgreSQL·현장 보안 검증이다.
